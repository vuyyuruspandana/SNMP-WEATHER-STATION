...
static char *pressure;

int my_scalar_pressure_handler(
    ...
    switch(reqinfo->mode) {
		...
		if (strncmp(line, "pressure", strlen("pressure")) == 0) {
			printf("line matched pressure\n");
			pressure = strstr(line, "\t");
 ...
 */
     ...
     * function. 
     */
    netsnmp_register_long_instance("my scalar_pressure variable",
                                  my_registration_oid,
                                  OID_LENGTH(my_registration_oid),
                                  pressure, &my_scalar_pressure_handler);

    DEBUGMSGTL(("scalar_pressure",
                "Done initalizing scalar_pressure\n"));
}
