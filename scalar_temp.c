...
init_scalar_temp(void)
{
     ...
     */
    DEBUGMSGTL(("scalar_temp",
                "Initalizing scalar_temp.  Default value = %ld\n",
                temp));
 ...
 */
     ...
     * function. 
     */
    netsnmp_register_long_instance("my scalar_temp variable",
                                  my_registration_oid,
                                  OID_LENGTH(my_registration_oid),
                                  &temp, &my_scalar_temp_handler);

    DEBUGMSGTL(("scalar_temp",
                "Done initalizing scalar_temp\n"));
}
