...
int my_scalar_humidity_handler(
    ...
    switch(reqinfo->mode) {
		...
		if (strncmp(line, "humidity", strlen("humidity")) == 0) {
			printf("line matched humidity\n");
			humidity = strstr(line, "\t");
 ...
 */
     ...
     * function. 
     */
    netsnmp_register_long_instance("my scalar_humidity variable",
                                  my_registration_oid,
                                  OID_LENGTH(my_registration_oid),
                                  humidity, &my_scalar_humidity_handler);

    DEBUGMSGTL(("scalar_huidity",
                "Done initalizing scalar_humidity\n"));
}
