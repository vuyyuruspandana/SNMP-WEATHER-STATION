...
int my_scalar_wind_handler(
    ...
    switch(reqinfo->mode) {
		...
		if (strncmp(line, "wind", strlen("wind")) == 0) {
			printf("line matched wind\n");
			wind = strstr(line, "\t");
			printf("wind value:%s\n", wind);
                 	break;
		 }
 ...
 */
     ...
     * function. 
     */
    netsnmp_register_long_instance("my scalar_wind variable",
                                  my_registration_oid,
                                  OID_LENGTH(my_registration_oid),
                                  wind, &my_scalar_wind_handler);

    DEBUGMSGTL(("scalar_wind",
                "Done initalizing scalar_wind\n"));
}
