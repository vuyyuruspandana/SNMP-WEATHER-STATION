...
int my_scalar_loc_handler(
    ...
    switch(reqinfo->mode) {
		...
		if (strncmp(line, "location", strlen("location")) == 0) {
			printf("line matched location\n");
			loc = strstr(line, "\t");
			printf("location value:%s\n", loc);
                 	break;
		 }
            }
 ...
 */
     ...
     * function. 
     */
    netsnmp_register_long_instance("my scalar_loc variable",
                                  my_registration_oid,
                                  OID_LENGTH(my_registration_oid),
                                  loc, &my_scalar_loc_handler);

    DEBUGMSGTL(("scalar_loc",
                "Done initalizing scalar_loc\n"));
}
